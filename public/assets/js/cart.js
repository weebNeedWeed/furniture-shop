...
function createItem(item_name) {
	...
	product_name.innerHTML = `Tên hàng: ${shopItem.vietnameseName}`;
    ...
    Số lượng: 
    <button class="adjust-quantity-btn" >
      <i class="fa-solid fa-plus"></i>
    </button>
    <input min=\"1\" type=\"number\"  id=${item_name.replace(
			/\s/g,
			"-",
		)} class=\"amount-input\" value=\"1\" disabled>
    <button class="adjust-quantity-btn" >
  ...
  `;
	...
	const handleIncrementQuantity = () => {
		_input.value = parseInt(_input.value, 10) + 1;
		calculateInfo();
	};
	product_quantity.getElementsByTagName("button")[0].onclick =
		handleIncrementQuantity;

	const handleDecrementQuantity = () => {
		const oldValue = parseInt(_input.value, 10);
	...
	product_quantity.getElementsByTagName("button")[1].onclick =
		handleDecrementQuantity;

	let product_remove = document.createElement("p");
	product_remove.className = "product-remove";
	product_remove.onclick = function () {
		removeItem(item_name.replace(/\s/g, "-"));
	};
